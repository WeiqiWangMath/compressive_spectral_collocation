...
%     m2(i) = randi([1 5]);
%     c(i) = rand(1);
%     u_exact = @(x)  u_exact(x) + c(i) * sin(2*pi * m1(i) * x(:,1)) .* sin(2*pi * m2(i) * x(:,2));
% end

% load exact sparse solution
...
save('data/D2_Eta1_SoluSparse.mat','s_vals','y_data','Sp','N');
% save('data/Exact_sparse.mat','u_exact','m1','m2','c')
